...
in terms of population health and economic consequences. For population health effects I took number of fatalaties and injuries caused by each event in account. For economic consequences I took property damage and crop damage expenses into account.
...
Finding most harmful events w.r.t economoci consequences by extracting events from the dataframe having economic cost greater than average.
...
ecnmc_cnsqnc_plot <-ggplot(data=ecnmc_cost_per_hrmfl_event_df, aes(x=Event, y=Economic_Cost_Billion)) +
  geom_bar(stat="identity") + xlab("Event") + ylab("Economic Cost ($) in billions") + coord_flip()
